...
\begin{enumerate}[a)]
    \item Praktik Percaloan: Menjadi sulit dilakukan karena tiket terus berubah, sehingga menyulitkan penjualan kembali tiket.
    \item \textit{Anti-Screenshot}: Tindak kejahatan melalui tangkapan layar dapat digagalkan karena gambar tiket akan kedaluwarsa dalam hitungan detik.
    \item Keamanan Data: Risiko eksfiltrasi data (\textit{Data Exfiltration}) dimitigasi melalui prinsip \textit{Data Minimization}, yaitu dengan mengatur \textit{payload} kode QR hanya memuat data teknis non-sensitif sehingga tidak ada informasi pribadi yang dapat dicuri.
    \item Validasi: Alat pemindai dapat melakukan validasi secara lebih fleksibel, didukung dengan mekanisme sinkronisasi asinkron (\textit{batching}) yang lebih efisien.
\end{enumerate}
...
\begin{enumerate}
    \item Mekanisme Pembangkitan Data (\textit{Data Generation}): Sistem saat ini menggunakan mekanisme \textit{Generate Once}, yaitu kode QR dibangkitkan sekali oleh server dan berlaku selamanya. Sebaliknya, sistem usulan menerapkan mekanisme \textit{Continuous Generation}, yaitu dengan mekanisme aplikasi klien membangkitkan kode baru secara berkala berdasarkan algoritma waktu (\textit{Time-Based}) dan kunci rahasia, sebagaimana ditunjukkan oleh indikator \textit{loop} pada model usulan.
    ...
    \item Mitigasi Risiko Keamanan: Seperti yang ditunjukkan oleh tanda silang merah pada model usulan (Gambar \ref{fig:model_usulan}), sistem baru secara desain memblokir vektor serangan umum:
    \begin{enumerate}[a)]
        \item Pencurian Tiket: Dicegah karena tidak ada berkas gambar statis yang tersimpan di galeri pengguna yang bisa dicuri.
        \item Penggandaan: Dicegah melalui mekanisme kedaluwarsa yang cepat, membuat salinan tiket menjadi tidak berguna (\textit{Denial of Service} bagi pihak ilegal).
    \end{enumerate}
\end{enumerate}

Berdasarkan analisis terhadap mekanisme pembangkitan, validitas, dan mitigasi risiko di atas, terlihat jelas adanya pergeseran paradigma keamanan dari sistem konvensional menuju sistem yang diusulkan. Untuk mempermudah pemahaman mengenai signifikansi perubahan tersebut, ringkasan komparatif antara karakteristik sistem saat ini (\textit{As-Is}) dengan sistem usulan (\textit{To-Be}) disajikan secara terstruktur pada Tabel \ref{tab:perbandingan_sistem}. Perbandingan ini menyoroti perbedaan fundamental pada aspek bentuk tiket, masa berlaku, hingga ketergantungan terhadap infrastruktur jaringan.
...
Perancangan alur proses berfokus pada mekanisme pembangkitan tiket dan validasi \textit{stateless} (nirkewarganegaraan) yang memungkinkan verifikasi dilakukan secara \textit{offline} tanpa mengorbankan keamanan. Sistem menerapkan pendekatan \textit{Hybrid Verification} yang menggabungkan Tanda Tangan Digital (untuk integritas data statis) dan TOTP (untuk validitas waktu dinamis).
...
% Sisipkan ini setelah poin 4 (Visualisasi QR) di subbab 4.3.1
\begin{figure}[htbp]
    \centering
    % Kamu perlu buat gambar flowchart proses generate di HP User
...
Mekanisme pembangkitan tiket yang dijelaskan pada diagram meliputi langkah-langkah berikut:
\begin{enumerate}
...
\begin{figure}[htbp]
    \centering
    % Masukkan diagram Flowchart Validasi Baru di sini
