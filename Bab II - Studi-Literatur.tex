...
Komponen visual utama kode QR dapat dilihat pada Gambar \ref{fig:struktur_qrcode}, dan untuk rincian dari \textit{function patterns} dijelaskan sebagai berikut:
...
Salah satu keunggulan teknis kode QR yang krusial untuk implementasi \textit{e-ticket} adalah kemampuan koreksi kesalahan menggunakan algoritma Reed-Solomon. Fitur ini memungkinkan data tetap dapat dipulihkan dan dibaca meskipun sebagian area simbol rusak atau kotor \cite{tiwari2016introduction}. Tingkat koreksi kesalahan dibagi menjadi empat level sebagaimana ditampilkan pada Tabel \ref{tab:error_correction}.
...
\begin{enumerate}[a)]
    ...
    \item Eksfiltrasi Data dan Pencurian Kunci (\textit{Data \& Key Exfiltration}): Serangan ini menargetkan penyimpanan lokal (\textit{local storage}) pada perangkat pengguna. \textcite{sung2015security} menjelaskan bahwa data sensitif yang disimpan di perangkat rentan diakses oleh perangkat lunak berbahaya (\textit{malware}). Dalam konteks \textit{e-ticket}, jika penyerang berhasil mencuri kunci rahasia pembangkit token (\textit{seed key}) atau data pribadi pengguna (\textit{PII}) yang tersimpan, mereka dapat melakukan pengambilalihan akun atau merekonstruksi tiket valid secara ilegal. Ancaman ini menuntut penerapan prinsip \textit{Data Minimization} dan penyimpanan kunci yang aman.
    
    \item Rekayasa Balik (\textit{Reverse Engineering}): Serangan ini menargetkan logika aplikasi pemindai atau aplikasi pengguna. Penyerang mencoba membongkar kode sumber aplikasi untuk menemukan kunci enkripsi statis (\textit{hardcoded keys}) atau algoritma validasi yang digunakan. Jika penyerang berhasil mendapatkan kunci utama (\textit{Master Secret}) dari aplikasi pemindai, mereka dapat memalsukan tiket valid secara massal tanpa terdeteksi oleh sistem \textit{offline}. Risiko ini menegaskan pentingnya perlindungan fisik dan logik pada perangkat pemindai.
\end{enumerate}
...
\begin{enumerate}[a)]
    \item \textit{Plaintext}: Ini adalah pesan atau data asli yang dapat dibaca (\textit{readable}) yang dimasukkan ke dalam algoritma sebagai input.
    \item Algoritma Enkripsi: Algoritma yang melakukan berbagai transformasi matematis terhadap \textit{plaintext} untuk mengubahnya menjadi bentuk yang tidak dapat dibaca.
    \item Kunci Publik dan Privat: Sepasang kunci yang telah dipilih sedemikian rupa sehingga jika salah satu digunakan untuk enkripsi, maka kunci pasangannya digunakan untuk dekripsi. Transformasi pasti yang dilakukan oleh algoritma bergantung pada kunci publik atau privat yang diberikan sebagai input.
    \item \textit{Ciphertext}: Pesan terenkripsi atau teracak yang dihasilkan sebagai output. \textit{Ciphertext} bergantung pada \textit{plaintext} dan kunci yang digunakan. Untuk pesan yang sama, dua kunci yang berbeda akan menghasilkan dua \textit{ciphertext} yang berbeda.
    \item Algoritma Dekripsi: Algoritma yang menerima \textit{ciphertext} dan kunci pasangan yang cocok (kunci privat jika dienkripsi dengan publik, atau sebaliknya), lalu menghasilkan kembali \textit{plaintext} asli.
\end{enumerate}
...
Proses pembuatan tanda tangan digital melibatkan penggunaan fungsi \textit{hash} dan kunci privat pengirim. Data atau pesan (\textit{message}) terlebih dahulu diproses melalui fungsi \textit{hash} untuk menghasilkan nilai ringkasan (\textit{digest}) yang unik. Nilai \textit{hash} ini kemudian dienkripsi menggunakan kunci privat pengirim untuk membentuk tanda tangan digital. Pada sisi penerima (verifikator), proses validasi dilakukan dengan mendekripsi tanda tangan menggunakan kunci publik pengirim untuk mendapatkan nilai \textit{hash} asli, dan membandingkannya dengan nilai \textit{hash} yang dihitung ulang dari data yang diterima. Jika kedua nilai tersebut identik, maka integritas dan keaslian data terjamin. Dalam penelitian ini, algoritma yang digunakan adalah \textit{Elliptic Curve Digital Signature Algorithm} (ECDSA), yang merupakan varian dari DSA (\textit{Digital Signature Algorithm}) yang beroperasi pada grup kurva eliptik.

\subsection{HMAC dan Derivasi Kunci (\textit{Key Derivation})}
\label{sec:hmac_kdf}

\textit{Hash-based Message Authentication Code} (HMAC) adalah konstruksi spesifik untuk menghitung \textit{message authentication code} (MAC) yang melibatkan fungsi \textit{hash} kriptografis yang dikombinasikan dengan kunci rahasia. Sesuai standar RFC 2104, HMAC menyediakan cara untuk memverifikasi integritas informasi sekaligus autentikasi asal pengirim.

Selain untuk autentikasi, properti pseudorandom dari HMAC membuatnya ideal digunakan sebagai Fungsi Derivasi Kunci atau \textit{Key Derivation Function} (KDF). NIST SP 800-108 merekomendasikan penggunaan HMAC dalam mode \textit{Counter} atau \textit{Feedback} untuk menurunkan kunci-kunci kriptografi baru dari sebuah kunci induk (\textit{Master Key}). Secara matematis, kunci turunan ($K_{derived}$) dapat dihasilkan dengan rumus:
\begin{equation}
    K_{derived} = HMAC(K_{master}, Context)
\end{equation}
Dalam arsitektur sistem yang terdistribusi, mekanisme ini memungkinkan entitas (seperti alat pemindai) untuk merekonstruksi kunci spesifik milik pengguna hanya dengan mengetahui ID pengguna ($Context$) dan Kunci Induk, tanpa perlu menyimpan basis data kunci yang besar. Konsep ini menjadi fondasi bagi mekanisme validasi \textit{stateless} dan \textit{offline}.
...
Penggunaan TOTP menjamin bahwa kode QR akan selalu berubah secara periodik mengikuti waktu server. Hal ini memitigasi risiko serangan penggandaan tiket (\textit{cloning attack}) dan serangan putar ulang (\textit{replay attack}) akibat penggunaan tiket hasil tangkapan layar yang telah kedaluwarsa.
...
Penyimpanan sementara atau \textit{caching} adalah teknik fundamental untuk efisiensi data. \textcite{tang2006benefit} menyatakan bahwa \textit{caching} data secara lokal pada node jaringan dapat secara signifikan meningkatkan efisiensi akses informasi dengan mengurangi latensi akses dan penggunaan \textit{bandwidth} jaringan. Dalam konteks validasi tiket, \textit{cache} lokal pada alat pemindai berfungsi untuk menyimpan riwayat transaksi dan status penggunaan tiket (misalnya: \textit{ticket\_id} X sudah masuk pukul Y). Hal ini memungkinkan proses pengecekan duplikasi (\textit{double-spending check}) tetap berjalan secara \textit{real-time} dan konsisten meskipun perangkat sedang tidak terhubung ke internet.
...
\subsection{Sistem kode QR Anti-Pemalsuan Berbasis \textit{Watermarking} dan CNN \cite{alsuhibany2025innovative}}
...
\subsection{Analisis Kerentanan Autentikasi Seluler Berbasis kode QR \cite{sung2015security}}
...
\subsection{Studi Komparasi kode QR Statis dan Dinamis \cite{yanuarafi2023perbandingan}}
...
Berdasarkan tinjauan literatur di atas, dapat dipetakan bahwa penelitian-penelitian terdahulu umumnya berfokus pada salah satu aspek keamanan secara terpisah. Belum banyak ditemukan sistem yang mengintegrasikan mekanisme pertahanan secara holistik untuk menjawab tiga kebutuhan utama keamanan tiket, yaitu: (1) Aspek dinamis untuk mencegah serangan penggandaan, (2) aspek privasi untuk melindungi data pengguna dari eksfiltrasi, dan (3) aspek integritas untuk menjamin keaslian penerbit tiket.

Penelitian ini bertujuan mengisi celah penelitian (\textit{research gap}) tersebut dengan mengusulkan arsitektur \textit{Dynamic Secure QR Code}. Kontribusi utama penelitian ini adalah penggabungan algoritma TOTP, Tanda Tangan Digital (ECDSA), dan mekanisme \textit{Key Derivation} yang memungkinkan validasi tiket dilakukan secara aman, cepat, dan \textit{offline} tanpa mengorbankan privasi data pengguna. Perbandingan posisi penelitian ini dengan penelitian terkait dapat dilihat pada Tabel \ref{tab:state_of_the_art}.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{ l >{\raggedright\arraybackslash}X c c c }
        \toprule
        \textbf{Peneliti} & \textbf{Fokus} \newline \textbf{Penelitian} & \textbf{Dinamis} & \textbf{Privasi} & \textbf{Integritas} \\
        \midrule
        ...
        \addlinespace
        \textbf{Penelitian Ini} & \textbf{Sistem \textit{E-Ticket} (TOTP + Data Minimization + TTD)} & \textbf{Ya} & \textbf{Ya} & \textbf{Ya} \\
        \bottomrule
    \end{tabularx}
\end{table}